...
[Published 0.5.1 version](https://chatgpt.com/plugins/plugins_6aa069eeb4408191a73c5eb02f19bbab) · [Releases](https://github.com/ailuntx/keybook/releases) · [Privacy](docs/privacy.md) · [Terms](docs/terms.md) · [Support](https://github.com/ailuntx/keybook/issues)
...
Development only: `npm ci --ignore-scripts`, then `npm test`. Source version 0.6.0 uses `keybook` as the plugin package name, directory, script and configuration path. The existing public listing still serves version 0.5.1; the new package identity requires a separate directory submission. The root package is private build/test tooling, not a distributable npm CLI. The build embeds the YAML dependency and includes its license. Tests exercise the plugin away from the source tree and node_modules.
