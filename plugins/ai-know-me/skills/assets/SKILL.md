...
`defaults` are fallback login settings, not evidence of an account on every website. Use a service-specific credential when present; do not guess which password variant applies, transform legacy prefixes, or try variants automatically. Do not implement browser autofill.
