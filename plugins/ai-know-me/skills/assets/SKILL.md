...
1. Check `command -v ai-know-me` and `ai-know-me --version`. Require 0.3.4 or newer, which removes plaintext credential output. If absent or older, install with `npm install --global ai-know-me@latest --ignore-scripts --registry=https://registry.npmjs.org` before using credentials.
