...
function document(text) {
  const docs = import_yaml.default.parseAllDocuments(text, { prettyErrors: false, strict: true, uniqueKeys: true, logLevel: "silent" });
  if (docs.length !== 1) throw new Error("\u53EA\u652F\u6301\u5355\u4E2A YAML \u6587\u6863");
  const doc = docs[0];
  if (doc.errors.length || doc.warnings.length) throw new Error("YAML \u683C\u5F0F\u65E0\u6548\uFF08\u4E0D\u56DE\u663E\u539F\u6587\uFF09");
  import_yaml.default.visit(doc, (_, node) => {
    if (import_yaml.default.isAlias(node) || node?.anchor || node?.tag) throw new Error("\u4E0D\u652F\u6301 YAML \u5F15\u7528\u3001\u951A\u70B9\u6216\u6807\u7B7E");
  });
...
async function resolveFile(file) {
  ...
  try {
    ...
    throw new Error("\u5C1A\u672A\u521D\u59CB\u5316\uFF1A\u4F7F\u7528\u968F\u63D2\u4EF6\u63D0\u4F9B\u7684\u811A\u672C\u6267\u884C init --file /absolute/path/\u6211\u7684\u5BC6\u94A5.yaml");
  }
}
...
function summaries(data, query = "") {
  query = query.toLowerCase();
  return Object.entries(data.assets).flatMap(
    ([group, entries]) => Object.keys(entries).map((name) => ({ group, name, path: `${group}.${name}` }))
  ).filter((entry) => entry.path.includes(query));
}
function resolveEntry(data, query) {
  query = query.toLowerCase();
...
async function run(data, mappings, command) {
  ...
  return new Promise((resolve) => {
    let child;
    try {
      child = spawn(command[0], command.slice(1), { env, shell: false, stdio: "ignore" });
    } catch {
      resolve({ status: "failed", exit_code: 1, message: "\u65E0\u6CD5\u542F\u52A8\u76EE\u6807\u7A0B\u5E8F\uFF1B\u8BF7\u68C0\u67E5\u7A0B\u5E8F\u53CA\u53C2\u6570" });
      return;
    }
...
try {
  if (process.argv[2] === "run") {
    ...
    const { data } = await read(await resolveFile(values.file));
    ...
    let result;
    if (command === "init") result = await init(v.file);
    else {
      const file = await resolveFile(v.file);
      const { data, stat } = await read(file);
      if (command === "list") {
        if (target && !Object.hasOwn(data.assets, target.toLowerCase())) throw new Error("\u5206\u7C7B\u4E0D\u5B58\u5728");
        result = summaries(data).filter((entry) => !target || entry.group === target.toLowerCase());
      } else if (command === "search") {
        if (!target) throw new Error("\u8BF7\u8F93\u5165\u641C\u7D22\u8BCD");
        result = summaries(data, target);
      } else if (command === "get") {
        if (!target) throw new Error("\u8BF7\u8F93\u5165\u540D\u79F0\u6216\u5206\u7C7B.\u540D\u79F0");
        result = get(data, target);
      } else result = { valid: true, file, groups: Object.keys(data.assets), entries: summaries(data).length, privatePermissions: (stat.mode & 63) === 0 };
    }
